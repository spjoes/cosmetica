...
public class CosmeticFeatureRenderer extends FeatureRenderer<AbstractClientPlayerEntity, PlayerEntityModel<AbstractClientPlayerEntity>> {
    ...
    public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, AbstractClientPlayerEntity player, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
        if(CosmeticaClient.getPlayerEntries().entries() != null && !player.isInvisible()){
            for (PlayerEntries.Entry.Entries entries : CosmeticaClient.getPlayerEntries().entries().entries) {
                if(entries.playerInformation.name.equals(player.getName().asString())){
                    for (String type : entries.cosmeticInformation.type.split("\\|")) {
                        ...
                        String modelId = "cosmetica:type#inventory".replace("type", type);
//                        String modelId = "minecraft:trident#inventory";
